...
def generate_pdf_report(project: dict, calculations: dict) -> bytes:
    ...
    # ── Generic fallback: render any module not already handled ──────
    _HANDLED = {"mosfet_losses", "gate_resistors", "input_capacitors", "thermal",
                "protection_dividers", "audit_log", "transparency", "waveform"}
    _SKIP_KEYS = {"_meta", "notes", "warnings", "curve", "checks", "time_ns",
                  "vgs", "vds", "id", "ig", "pd", "annotations", "params_used",
                  "check_count", "compatibility_text", "model_note", "guardrails",
                  "clamp_activity", "has_motor_data"}

    section_num = 7
    for mod_key, mod_data in calculations.items():
        if mod_key in _HANDLED or not isinstance(mod_data, dict):
            continue
        # Collect scalar/bool values only
        rows = [["Parameter", "Value"]]
        for k, v in mod_data.items():
            if k in _SKIP_KEYS or isinstance(v, (dict, list)):
                continue
            display_key = k.replace("_", " ").replace(" v ", " V ").title()
            display_val = ("✓" if v is True else "✗" if v is False else str(v)) if not isinstance(v, float) else f"{v:.4g}"
            rows.append([display_key, display_val])
        if len(rows) < 2:
            continue
        label = mod_key.replace("_", " ").title()
        story.append(Paragraph(f"{section_num}. {label}", h1_style))
        section_num += 1
        # Emit warnings as a note line before table
        warn_list = mod_data.get("warnings", [])
        if warn_list:
            for w in warn_list[:3]:
                story.append(Paragraph(f"⚠ {w[:200]}", small))
        story.append(_make_table(rows))
        story.append(Spacer(1, 5*mm))

    story.append(Spacer(1, 10*mm))
    story.append(HRFlowable(width="100%", thickness=1, color=GRAY))
    story.append(Paragraph("Generated by MC Hardware Designer | Gemini-powered extraction", small))
...
def _build_bom(calculations, project=None) -> list:
    ...
    if "snubber" in calculations:
        sn = calculations["snubber"]
        cs_pf = sn.get("cs_recommended_pf", 470)
        rs_ohm = sn.get("rs_recommended_ohm", 10)
        v_sw = int(sn.get("v_sw_peak_v", 70)) + 20
        add("Snubber Cap (Cs)", "RC snubber, switch node", f"{cs_pf} pF / {v_sw}V", 6, "C0G/NP0, 0603", "~$0.08")
        add("Snubber Resistor (Rs)", "RC snubber damping", f"{rs_ohm} Ω", 6, "0603, 0.25W, non-inductive", "~$0.03")
