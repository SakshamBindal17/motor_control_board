...
class PcbTraceThermalMixin:
    ...
    def calc_pcb_trace_thermal(self) -> dict:
        ...
        trace_w_mm = float(p.get("trace_width_mm", 7))
        trace_l_mm = float(p.get("trace_length_mm", 20))
        ...
        air_vel    = float(p.get("air_velocity_ms", 1))
        hs_theta   = float(p.get("hs_theta_sa", 5))
        hs_tint    = float(p.get("hs_theta_int", 0.5))
        hs_area    = float(p.get("hs_contact_area_cm2", 10))
        n_ext      = _clamp(int(float(p.get("n_external_layers", 2))), 0, 2)
        n_int      = _clamp(int(float(p.get("n_internal_layers", 0))), 0, 20)
        vias_on    = bool(p.get("vias_on", True))
        n_vias     = _clamp(int(float(p.get("n_vias", 10))), 1, 500)
        via_drill  = float(p.get("via_drill_mm", 0.3))
        via_plate  = float(p.get("via_plating_um", 25))
        plane_dist = float(p.get("plane_dist_mm", 0))
        copper_fill = float(p.get("copper_fill_pct", 0))
